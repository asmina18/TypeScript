...
const getPercentage = (tal: number, procent: number): number => (procent * tal) / 100;
...
const getElement = (arr: string[], substring: string): string | undefined => arr.find(element => element.includes(substring));

// Funktion til at hente et element i et array baseret på indeks
const getElementAtIndex = (arr: string[], index: number): string | undefined => (index >= 0 && index < arr.length) ? arr[index] : (console.error('Index out of bounds!'), undefined);
